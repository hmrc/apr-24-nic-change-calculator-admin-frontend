...
class CalculationConnector @Inject()(config: Configuration, httpClient: HttpClientV2)
  ...
  private def fullUrl(from: Option[Instant], to: Option[Instant]) = url"$baseUrl/apr-24-nic-change-calculator/summary?from=${from.map(dateFormat.format)}&to=${to.map(dateFormat.format)}"
